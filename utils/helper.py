...
def create_dataset(hypes, train=True, real=False):
    ...
    if train:
        ...
        transform_operation = transforms.Compose([RandomCropSame(256), RandomAffine(), RandomFlip(), RandomBlur(), TolABTensor()])
        ...
        val_dataset = OldPhotoDataset(hypes['val_file'],
                                      transform=transforms.Compose([RandomCropSame(256), RandomAffine(),RandomFlip(),RandomBlur(), TolABTensor()]))
